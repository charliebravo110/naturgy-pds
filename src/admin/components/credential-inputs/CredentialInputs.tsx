...
const CredentialInputs = (props: any) => {
  const { credentialType, setCredentialType, credential, setCredential, error, selectedInput, selectInput } = props
  ...
  useEffect(() => {
    if (selectedInput === 2 || selectedInput === 3 || selectedInput === 4 || selectedInput === 5) {
      setInactive(true)
    } else {
      setInactive(false)
    }
