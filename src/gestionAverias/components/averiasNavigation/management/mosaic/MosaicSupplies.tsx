...
import useStyles, { StyledTableCell } from '../list/List.styles'
import Pagination from '../../../../../common/components/pagination/Pagination2'

const MosaicSupplies = (props: any) => {
	const {
		listItems,
		setFinalList,
		currentPage,
						...
						listItems.length === 0 ?
							...
							listItems.slice(
								...
								(supply, index) => (
                                    ...
                                    <div className={classes.title}>{t('averias.management.searchCups.table.nif')}</div>
                                    <div className={`${classes.value}`}>{supply.holderDocumentNumber}</div>
                                     </Grid>

                                     <Grid style={{ textAlign: 'center',alignContent:'center' }}  className={classes.rowMosaic}>
                                    <div className={classes.title}>{t('averias.management.searchCups.table.name')}</div>
                                    <div className={`${classes.value}`}>{supply.holderName}</div>
                                     </Grid>

                                     <Grid style={{ textAlign: 'center',alignContent:'center' }}  className={classes.rowMosaic}>
                                    <div className={classes.title}>{t('averias.management.searchCups.table.cups')}</div>
                                    <div className={`${classes.value}`}>{supply.cups}</div>
                                     </Grid>

                                     <Grid style={{ textAlign: 'center',alignContent:'center' }}  className={classes.rowMosaic}>
                                    <div className={classes.title}>{t('averias.management.searchCups.table.address')}</div>
                                    ...
                                    <span>
                                     ...
                                     <Grid style={{ textAlign: 'center',alignContent:'center' }}  className={classes.rowMosaic}>
                                    <div className={classes.title}> {t('averias.management.searchCups.table.suministro')}</div>
                                    <div className={`${classes.value}`}>
                                    {(supply.isGenerator === '1') ?
                                     ...
                                     <Grid style={{ textAlign: 'center',alignContent:'center' }}  className={classes.rowMosaic}>
                                     {
													<div style={{display:'grid'}} className={classes.editButton} onClick={() => handleClick(supply)}>
...
export default withRouter(MosaicSupplies)
