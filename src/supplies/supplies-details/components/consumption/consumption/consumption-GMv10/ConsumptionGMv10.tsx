...
import useStyles from './ConsumptionGMv10.styles'
...
const ConsumptionGMv10 = (props: any) => {
  ...
  const handleExport = () => {
    ...
    dispatch(thunkGetCurrentSupplyConsumptions(false, supplyData.holderDocumentNumber, supplyData.cups, granularity, auxStartDate, endDate, null, supplyData.isGenerator, supplantedUser, isDelegate, setIsLoading, (supplyData.isSelfConsumption && supplyData.isSelfConsumption.cau && supplyData.isSelfConsumption.cau != '') ? '1' : '0', supplyData.measurementSystem, (response) => {
      if (response == 'showModalError1007') {
        ...
        if (compare === 'C') {
          dispatch(thunkGetCurrentSupplyConsumptions(false, supplyData.holderDocumentNumber, supplyData.cups, granularity, auxStartDateCompare, endDateCompare, null, supplyData.isGenerator, supplantedUser, isDelegate, setIsLoading, (supplyData.isSelfConsumption && supplyData.isSelfConsumption.cau && supplyData.isSelfConsumption.cau != '') ? '1' : '0', supplyData.measurementSystem, (response) => {
            if (response == 'showModalError1007') {
              setModalMessage('errors.autoconsumo.1007')
              setModalSecondMessage('')
              toggle();
              ...
              if (fileType === 'excel') {
                response && response.map(
                  (item) => {
                    return item.consumptions && item.consumptions.items && item.consumptions.items.map(
                      (consumption) => {
                        let auxConsumptionValueCompare

                        if (isGeneration) {
                          if (isGenerationTab) {
                            if (consumptionsFilters.showR2 === 'S')
                            {
                              auxConsumptionValueCompare = consumption.reactive2
                            }
                            else if (consumptionsFilters.showR3 === 'S')
                            { 
                              auxConsumptionValueCompare = consumption.reactive3
                            }
                            else
                            {
                              auxConsumptionValueCompare = consumption.activeOutput
                            }
                          } else {
                            if (consumptionsFilters.showR2 === 'S')
                            {
                            ...
                            { 
                              auxConsumptionValueCompare = consumption.activeInput
                            }
                          }
                        } else {
                          if (consumptionsFilters.showR1 === 'S')
                          {
                            auxConsumptionValueCompare = consumption.reactive1
                          }
                          else if (consumptionsFilters.showR4 === 'S')
                          {
                            auxConsumptionValueCompare = consumption.reactive4
                          }
                          else
                          {
                            auxConsumptionValueCompare = consumption.consumptionValue
                          }
                        }

                        if (auxConsumptionValueCompare) {
                          auxConsumptionsData.push({
                            cups: supplyData.cups ? supplyData.cups : '0',
                            date: consumption.consumptionDate ? consumption.consumptionDate : '00/00/0000',
                            hour: consumption.hour ? consumption.hour : '0',
                            consumption: auxConsumptionValueCompare ? auxConsumptionValueCompare.replace(',', '.') : '0.000',
                            obtainingMethod: consumption.readingType === 'R' ? t('supplies.suppliesDetails.components.consumption.exportDialogs.real') : t('supplies.suppliesDetails.components.consumption.exportDialogs.estimation')
                          })
                        }
                      ...
                      (consumption) => {
                        ...
                        if (auxConsumptionValueCompare) {
                          let auxConsumptionsItem = [
                            ...
                            consumption.consumptionDate ? consumption.consumptionDate : '00/00/0000',
                            consumption.hour ? consumption.hour : '0',
                            auxConsumptionValueCompare ? auxConsumptionValueCompare.replace(',', '.') : '0.000',
                            consumption.readingType ? consumption.readingType : '0'
                          ]
  ...
  return (
    ...
    <Grid>
      ...
      >
        <Grid item>
          {t('supplies.suppliesDetails.components.consumption.consumption.displayConsumptionGMv10-1-consumption')}
        </Grid>
        <Grid item>
      ...
      <Grid container md={8} sm={10} xs={12} className={classes.telemedidoAdviseBox}>
        <Grid item md={12} sm={12} xs={12}>
          <Grid container justifyContent='center' alignItems='center' className={classes.telemedidoAdviseContainer}>
            <Grid container sm={12} md={11} lg={8}>
              <Grid container alignItems='center'>

                <Grid item md={1} sm={2} xs={12}>
                  <img src={InfoIcon} className={classes.infoIcon} alt='' />
                </Grid>
                <Grid item md={11} sm={10} xs={12} className={classes.telemedidoAdviseTitle}>
                  <div>{t('supplies.suppliesDetails.components.consumption.charts.charts.telemedido')}</div>
                </Grid>
...
export default ConsumptionGMv10
