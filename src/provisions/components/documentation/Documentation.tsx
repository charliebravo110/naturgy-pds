...
const Documentation = (props: any) => {
  ...
  const { state, isLoading, setIsLoading, blockerDocumentsList, indAceptoFacturaDigital, setIndAceptoFacturaDigital, showDocOnCancelDossier, setIsLoadingGlobal } = props
  ...
  const [provisionalDoc, setProvisionalDoc] = useState()
    ...
    if (Refresh === true) {
      //setTimeout(() => {
        ...
        dispatch(thunkGetProvision(currentProvision.dossierCod, defaultName, (response) => {
          if (response) {
            console.log(response,'RVD')
             dispatch(setCurrentProvision(response))
            
            setNewDocumentsRecieved && setNewDocumentsRecieved({
              sentDocument: response.documentList && response.documentList.sentDocument ? response.documentList.sentDocument.map((item) => item) : [],
  ...
  useEffect(() => {
    if (currentProvision.dossierStatusId === 'STATUS0003' ||
      ...
      currentProvision.dossierStatusId === 'STATUS0001') {
              ...
              <Grid container direction='column'>
                { currentProvision.idDossierTypeId === 'DOSTYP001' &&
                  <Grid
                    ...
                    alignItems='center'
                    className={classes.documentationExpedientContainer}
                  >
                    <img className={classes.picture} src={InfoIcon} alt='' />
                    <Grid container lg={10} md={10} sm={11}>
                      <Grid container>
                        <Grid item md={12} sm={12} xs={12} className={classes.documentationExpedientText}>
                          <span>
                            {t('provisions.documentation.documentationExpedient.documentationExpedientText')}
                            {' '}
                            <a className={classes.otherDocAdviseTitleLink} href={t('provisions.documentation.documentationExpedient.documentationExpedientUrl')} target='_blank'>
                              {t('provisions.documentation.documentationExpedient.documentationExpedientUrlText')}
                            </a>
                          </span>
                        </Grid>
                      </Grid>
                  ...
                  <Grid container spacing={2}>
                    {
                      !isLoading && documentsList.map((item, index) => (
                        item &&
                        (
                          (item.documentType !== 'DOCTYP0203' && item.documentType !== 'DOCTYP0302') ||
                          (
                ...
                >
                  <Grid item md={1} sm={1} xs={1} className={classes.otherDocAdviseIcon}>
                    <img src={AlertIcon} className={classes.alertIconRevision} alt='' />
                  </Grid>
                  <Grid container lg={11} md={11} sm={11}>
                    <Grid container alignItems='center'>
                      <Grid item md={12} sm={12} xs={12} className={classes.otherDocAdviseTitle}>
