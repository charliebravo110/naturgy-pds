...
import { thunkGetMasterData, thunkUpdateDossier } from '../../../store/actions/ProvisionsThunkActions'
...
import { thunkExpedientForm } from '../../../store/actions/ProvisionsThunkActions';
...
const Steps = (props: any) => {
  ...
  const [isOnDeck, setIsOnDeck] = useState(null)
  const [fotovoltaicaSuperficie, setFotovoltaicaCubierta] = useState(null)
  const [typeAutoconsumo, setTypeAutoconsumo] = useState('')
  const [typeConexion, setTypeConexion] = useState('')
  ...
  const [isSelfConsumption, setIsSelfConsumption] = useState(false)
  const [datosUser, setDatosUser] = useState({ isOwner: false, docRepresentante: '', preType: false, isFehaciente: '0' })
  ...
  const handleSendExp = async (openingDate, endDate, attempts, expedientType, busZeusAttempts, tr9callOK, errorDirection ) => {
  ...
  // El campo dossierSubtype viene informado inicialmente como DOSSUB000 cuando pinchamos en "Quiero una conexión de autoconsumo" y undefined cuando
  // seleccionamos "Quiero una nueva conexión de generación". Usamos esto para diferenciar el alta de generación de la de autoconsumo
  useEffect(() => {
    if (dossierSubtype && dossierSubtype === 'DOSSUB000') {
      setIsSelfConsumption(true)
    }
  }, [])
              ...
              <Grid item md={11} sm={11} xs={12} justifyContent='center'>
                {
                  ...
                  (requestDataCompleted || (currentProvision.dossierCod !== '' && currentProvision.dossierStatusId === 'STATUS0001')) ?
                    ...
                    <RequestDataWelcome
                      ...
                      resetDatosUser={resetDatosUser}
                      setIsOnDeck={setIsOnDeck}
                      setFotovoltaicaSuperficie={setFotovoltaicaCubierta}
