...
import useStyles from './Building.styles'
...
const Building = (props: any) => {
  ...
  const getDefaultTechData = () => {
    ...
    } else if (dossierSubtype === 'DOSSUB001') {
      techDataDefault = {
        timeDiscrimination: '0',
        sendExtensionBudgetInd: '0',
      }
    } else if (dossierSubtype === 'DOSSUB031') {
      techDataDefault = {
    ...
    else if (dossierSubtype === 'DOSSUB006' || dossierSubtype === 'DOSSUB005') {
      techDataDefault = {
        idDossierTensionType: '',
        idDossierTensionSubtype: '',
        phase: '',
  ...
  // Comprobar si el subtipo es Bloque de pisos o Grupo de viviendas unifamiliares para eliminar Alta del select de tensiones
  useEffect(() => {
    if (provisions.selectedSupplyType === 'UR' && (provisions.dossierSubtype === 'DOSSUB005' || provisions.dossierSubtype === 'DOSSUB006') && voltageSupplySelect.length > 1 && !currentProvision.techData) {
  ...
  // Si viene un solo item se setea como defecto en el nuevo numberOfPhasesSelect cargado excepto grandes desarrollos
  useEffect(() => {
    if (!currentProvision.techData && dossierSubtype !== 'DOSSUB001') {
  ...
  // Si viene un solo item se setea como defecto en el nuevo voltageSupplySelect cargado excepto grandes desarrollos
  useEffect(() => {
    if (!currentProvision.techData && dossierSubtype !== 'DOSSUB001') {
  ...
  // Si viene un solo item se setea como defecto en el nuevo voltageSelect cargado excepto grandes desarrollos
  useEffect(() => {
    if (!currentProvision.techData && dossierSubtype !== 'DOSSUB001') {
  ...
  useEffect(() => {
    for (let i = 0; i < powerListErrors.length; i++) {
      if (powerListErrors[i] == null) {
        powerListErrors[i] = {}
      }
    }
  ...
  useEffect(() => {
    if (provisions.dossierSubtype !== 'DOSSUB002' &&
      provisions.dossierSubtype !== 'DOSSUB003' &&
      provisions.dossierSubtype !== 'DOSSUB004' &&
      provisions.dossierSubtype !== 'DOSSUB005' &&
      provisions.dossierSubtype !== 'DOSSUB005' &&
      provisions.dossierSubtype !== 'DOSSUB007' &&
      provisions.dossierSubtype !== 'DOSSUB008') {
  ...
  return (
    <Grid container justifyContent='space-between'>
      ...
      <Grid container className={classes.tablePadding}>
        <div className={classes.tableTitle}>{t('provisions.newProvision.requestData.supplyType.form.technicalInstallationData.building.table.title')}</div>
          ...
          <Grid container className={classes.addSupplie}>
            <div onClick={handleAddSupplie}>
              <IconTextButton
                ...
                text={t(
                  'provisions.newProvision.requestData.supplyType.form.technicalInstallationData.building.table.addSupplie'
                )}
              />
            </div>
          </Grid>
              ...
              <Grid container>
                <div className={classes.totalMobileContainer}>
                  <Grid container>
                    ...
                    <Grid container className={classes.totalRow} justifyContent='space-between'>
                      <Grid item className={classes.totalSubtitle}>{t('provisions.newProvision.requestData.supplyType.form.technicalInstallationData.family.table.totals.area')}</Grid>
                      <Grid item className={classes.totalVariables}>{totalBuildableArea}</Grid>
                    </Grid>
                    <Grid container justifyContent='space-between'>
