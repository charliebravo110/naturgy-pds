...
const ListItem = (props: any) => {
  ...
  const handlePowerRequestedMin = (power, index) => {
    // Estas tipologias no tienen minimo de potencia requerida 
    if (provisions.dossierSubtype !== 'DOSSUB029' &&
      ...
      //Comprobamos la potencia minima a controlar por superficie o Num de suministros
      let power1 = 0
      let power2 = 0
        ...
        setPowerListErrors(powerListErrors.map((item, i) => {
          ...
          if (index === i && powerList && powerList[i]) {
            if (!isNaN(power)) {
              ...
              //No admitir P> a potencia minima calculada kW excepto tipo uso industria 1 suministro
    ...
    <>
      ...
      <TableRow key={index} className={classes.row}>
        {dossierSubtype !== 'DOSSUB007' &&
          //ppm 1007821 - inicio
          provisions.dossierSubtype !== 'DOSSUB029' &&
          provisions.dossierSubtype !== 'DOSSUB030' &&
          //ppm 1007821 - fin
          dossierSubtype !== 'DOSSUB009' && (
            <StyledTableCell>
              <div className={`${classes.inputTableFirst} ${powerListErrors[index] && powerListErrors[index].applicationType && powerListErrors[index].applicationType.bool && classes.errorInputPadding}`}>
                {state === 0 ? (
                  <Select
                    fullWidth
                    label={
                    ...
                    values={
                      provisions.selectedSupplySubtype === 'DOSSUB010' ?
                        ...
                        :
                        typesSelect.filter(i => !i.startsWith('UTITYP007'))
                    }
                    codFiltering
                    disabled={
                      dossierSubtype === 'DOSSUB002' ||
          ...
          )}
        {
          ...
          <StyledTableCell>
            <div className={`${classes.inputTableFirst} ${powerListErrors[index] && powerListErrors[index].useType && powerListErrors[index].useType.bool && classes.errorInputPadding}`}>
              {
                state === 0 ?
                  <Select
                    fullWidth
